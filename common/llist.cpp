...
{
	LNode *entry = (LNode*)malloc(sizeof(LNode));
	entry->data = data;
...
llist_add_at(LList *list, uint32_t index, void *data)
{
	...
	entry = (LNode*)malloc(sizeof(LNode));
	entry->data = data;
...
{
	LList *copy = (LList*)malloc(sizeof(LList));
	LNode *entry;
	LNode *previous = NULL;
	...
	for (entry = list->first; entry != NULL; entry = entry->next) {
		LNode *new_entry = (LNode*)malloc(sizeof(LNode));
...
{
	void **array = (void**)malloc((list->size+1) * sizeof(void *));
	LNode *entry = list->first;
	uint32_t index = 0;
...
llist_iterator_remove(LListIterator *it)
{
	...
	internal_error("Called iterator_remove before first iterator_next");
